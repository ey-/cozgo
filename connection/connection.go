...
func StartListen(serviceAddress string) {
	RemoteAddr, _ := net.ResolveUDPAddr("udp", serviceAddress)
	...
	startLoop(conn, RemoteAddr)
}

func startLoop(connection *net.UDPConn, RemoteAddr *net.UDPAddr) {
...
func toForward(connection *net.UDPConn, remAddr *net.UDPAddr) {
	...
	binary.Write(messageBuffer, binary.LittleEndian, m8f)
	message := packet(messageBuffer.Bytes())
	...
	for {
		...
		if connectionReady && resetSend {
			...
			case bytes.Equal(c, []byte{27, 91, 65}): // up
				...
				go toForward(connection, remAddr)
