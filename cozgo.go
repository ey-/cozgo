...
func toLeft(connection *net.UDPConn, remAddr *net.UDPAddr) {
    ...
    if a > 5 && a < 8000 {
        ...
        //binary.Write(messageBuffer, binary.LittleEndian, []byte{0x04, 0x02, 0x00, 0x9e, 0x01})
        //binary.Write(messageBuffer, binary.LittleEndian, []byte{0x04, 0x0b, 0x00, 0x33, 0x00, 0x00, 0x20, 0x41, 0x00, 0x00, 0x00, 0x00, 0xfe, 0xff})
        // 00 40 slow, forward.
        //binary.Write(messageBuffer, binary.LittleEndian, []byte{0x04, 0x0b, 0x00, 0x33, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0})
        // 543200
        ...
        // 94e70000
        // lift goes up. veryy slow...
        //binary.Write(messageBuffer, binary.LittleEndian, []byte{0x04, 0x04, 0x00, 0x94, 0x00, 0x00, 0x80})

        // 990000ff7f
        // forward
        // binary.Write(messageBuffer, binary.LittleEndian, []byte{0x04, 0x05, 0x00, 0x99, 0x10, 0x00, 0x00, 0xff})
 
        //3914f529be368da7c000002041c2b8b23d00000043
        // moves to a defined position.
...
func streamVideo(videoStream chan []byte) {
    ...
    defer window.Close()
    for byt := range videoStream {
        imag, err := webp.DecodeRGBA(byt, options)
        if err != nil {
            panic(err)
        }
        img, _ := gocv.NewMatFromBytes(240, 240, gocv.MatTypeCV8U, imag.Pix)
        defer img.Close()
        window.IMShow(img)
        window.WaitKey(1)
    }
